        ...
        for entry in feed.entries:
            ...
            try:
                ...
                if entry.published.endswith('GMT'):
                    ...
                    parsed_date = datetime.strptime(entry.published, '%a, %d %b %Y %H:%M:%S Europe/Budapest')
            ...
            with connection_pool.get_connection() as connection:
                if not check_url_exists(connection, clean_url) and not skip_url(clean_url):
                    init_news(connection, 'rss', entry.link, clean_url, newspaper['name'], newspaper['id'], None, pub_time)
                    articles_found += 1
