...
def filter_list_end_biased(input_list, min_prob=0.3, max_prob=1.0):
    ...
    return [
        ...
        if random.random() < get_probability(index)
    ]


positive_dataset = Dataset.from_list(
    filter_list_end_biased(
        load_dataset("K-Monitor/kmdb_base")
        .remove_columns(
            [
                "news_id",
                "archive_url",
        ...
        .rename_column("source_url", "url")
        .rename_column("pub_time", "date")
        .map(lambda row: {"label": 1, "is_hand_annoted": True})["train"]
    )
)
...
negative_ds_prod = Dataset.from_list(rows).map(
    lambda row: {
        "keywords": [""],
...
negative_ds = Dataset.from_list(rows).map(
    lambda row: {
        "keywords": [""],
        "is_hand_annoted": True,
        "score": None,
